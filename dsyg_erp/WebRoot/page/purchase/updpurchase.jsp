...
*/		
	...
	function setPurchaseItemList() {
		...
		for(var i = 0; i < rows.length; i++) {
			...
			td.appendChild(createInput("tmpUpdPurchaseItemList[" + i + "].res01", sumicode));
			...
			if(res02 == "1" && theme1=="01") {
				...
				td.appendChild(createInput("tmpUpdPurchaseItemList[" + i + "].res03", cuprice));
			} else {
				td.appendChild(createInput("tmpUpdPurchaseItemList[" + i + "].res03", ""));
			}
			...
			td.appendChild(createInput("tmpUpdPurchaseItemList[" + i + "].makearea", makearea));
	...
	function tmpCalcAmount() {
		...
		//采购金额不含税
		$("#totalamount").val(all.toFixed(2));
		$("#tmpTotalamount").val(all.toFixed(2));
...
//		var supplierid = $("#supplierid").val().trim();
	...
	function changeThis(obj) {
		if(obj.checked) {
			var msg = "确定终了采购单？";
			var res02 = ${updPurchaseDto.res02};
			if(res02 == "1") {
				msg = "确定作废询价？";
			}
			if(confirm(msg)) {
					...
					<table width="100%" border="0" cellpadding="5" cellspacing="0">
						...
						<tr>
							...
							<td>
								<div class="box1_left"></div>
								<div class="box1_center">
									<s:textfield name="updPurchaseDto.res03" id="res03" maxlength="10" cssStyle="width:300px;" theme="simple"></s:textfield>
								</div>
								<div class="box1_right"></div>
							</td>
							<td align="right">
											...
											<s:iterator id="updPurchaseItemList" value="updPurchaseItemList" status="st1">
												...
												<s:else>
													...
													<td>
														...
														<s:elseif test='%{updPurchaseItemList[#st1.index].packaging == "2"}'>样品</s:elseif>
														<s:else>
