...
<script type="text/javascript">
	...
	function showDetail() {
		...
		if(id == "") {
			...
			var url = "<%=request.getContextPath()%>/salesreport2/showUpdSalesReport2ItemAction.action?updateSalesReportNo=" + id + "&date=" + encodeURI(new Date());
	...
	function add() {
		document.mainform.action = '<c:url value="/salesreport2/showAddSalesReport2Action.action"></c:url>';
		document.mainform.submit();
	}
	...
	function upd() {
		...
		if(id == "") {
			...
			document.mainform.action = '<c:url value="/salesreport2/showUpdSalesReport2Action.action"></c:url>' + "?updateSalesReportNo=" + id;
			document.mainform.submit();
		}
	}
	...
	function del() {
		...
		if(id == "") {
			...
			if(confirm("确定删除该记录吗？")) {
				document.mainform.action = '<c:url value="/salesreport2/delSalesReport2Action.action"></c:url>' + "?delSalesReportNo=" + id;
				document.mainform.submit();
			}
		}
	}
	...
	function exportExcel() {
		document.mainform.action = '<c:url value="/salesreport2/exportSalesReport2Action.action"></c:url>';
		document.mainform.submit();
	}
	...
	function changePage(pageNum) {
		...
		document.mainform.action = '<c:url value="/salesreport2/turnSalesReport2Page.action"></c:url>';
		document.mainform.submit();
	}
	...
	function changepagesize(pagesize) {
		...
		document.mainform.action = '../salesreport2/querySalesReport2List.action';
		document.mainform.submit();
	}
