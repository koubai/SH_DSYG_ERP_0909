...
<script type="text/javascript">
	...
	function del() {
		...
		if(id == "") {
			...
			if(confirm("确定删除吗？")) {
				document.mainform.action = "../cuprice/delCuPriceAction.action?delPriceCodeId=" + id;
				document.mainform.submit();
			}
		}
	}
	...
	function setQueryDate() {
		$("#strSetdateLow").attr("value", $("#setdateLow").val());
		$("#strSetdateHigh").attr("value", $("#setdateHigh").val());
	}

	//查询数据
	function queryList() {
		setQueryDate();
		document.mainform.action = '../cuprice/queryCuPriceAction.action';
		document.mainform.submit();
	}
	...
	function changePage(pageNum) {
		...
		document.mainform.action = '../cuprice/turnCuPriceAction.action';
		document.mainform.submit();
	}
	...
	<div id="containermain">
		<div class="content">
			...
			<s:form id="mainform" name="mainform" method="POST">
				...
				<s:hidden name="strSetdateLow" id="strSetdateLow"/>
				<s:hidden name="strSetdateHigh" id="strSetdateHigh"/>
				<div class="searchbox">
					<div class="box1">
						<label class="pdf10">设置日期</label>
						<div class="box1_left"></div>
						<div class="box1_center date_input">
							<input type="text" disabled="disabled" style="width: 105px;" id="setdateLow" value="<s:property value="strSetdateLow"/>" maxlength="10" />
							<a class="date" href="javascript:;" onclick="new Calendar().show(document.getElementById('setdateLow'));"></a>
						</div>
						<div class="box1_right"></div>
						<label>-</label>
						<div class="box1_left"></div>
						<div class="box1_center date_input">
							<input type="text" disabled="disabled" style="width: 105px;" id="setdateHigh" value="<s:property value="strSetdateHigh"/>" maxlength="10" />
							<a class="date" href="javascript:;" onclick="new Calendar().show(document.getElementById('setdateHigh'));"></a>
						</div>
						<div class="box1_right"></div>
					</div>
					...
					<div class="box1" style="margin-top:-3px; margin-left: 100px; color: red;">
						<s:actionmessage />
					</div>
					...
					<div class="tab_content" style="height: <s:property value="intPageSize * 35"/>px;">
						<table class="info_tab" width="100%" border="1" cellpadding="5" cellspacing="0">
							<tr class="tittle">
								<td width="20"></td>
								<td width="30">序号</td>
								<td width="120">铜价区间</td>
								<td width="60">设置时间</td>
								<td width="80">备注</td>
							</tr>
							<s:iterator id="cuPriceList" value="cuPriceList" status="st1">
								...
								<s:else>
									...
									<td>
										<s:iterator id="cuPriceDict01List" value="cuPriceDict01List" status="st3">
											<s:if test="%{cuPriceDict01List[#st3.index].code == cuPriceList[#st1.index].cu_price_code}">
