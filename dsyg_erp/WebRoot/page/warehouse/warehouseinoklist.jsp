...
<script type="text/javascript">
	...
	//查询数据
	function queryList() {
		document.mainform.action = '../warehouse/queryWarehouseInOkAction.action';
		document.mainform.submit();
	}
	...
	function changePage(pageNum) {
		...
		document.mainform.action = '../warehouse/turnWarehouseInOkAction.action';
		document.mainform.submit();
	}
								...
								<s:else>
									...
									<td>
										<s:iterator id="colorList" value="colorList" status="st3">
											<s:if test="%{colorList[#st3.index].code == warehouseInOkList[#st1.index].color}">
												<s:property value="fieldname"/>
											</s:if>
										</s:iterator>
									</td>
									<td>
										<s:if test='%{warehouseInOkList[#st1.index].packaging == "0"}'>整箱</s:if>
										<s:elseif test='%{warehouseInOkList[#st1.index].packaging == "1"}'>乱尺</s:elseif>
										<s:else>
