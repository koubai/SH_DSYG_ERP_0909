...
<script type="text/javascript">
	...
	function queryList() {
		setQueryDate();
		document.mainform.action = '../untaxinfo/queryUntaxInfoAction.action';
		document.mainform.submit();
	}
	...
	function changePage(pageNum) {
		...
		document.mainform.action = '../untaxinfo/turnUntaxInfoAction.action';
		document.mainform.submit();
	}
				...
				<div class="data_table" style="padding:0px;">
					...
					<div class="tab_content" style="height: <s:property value="intPageSize * 35"/>px;">
						<table class="info_tab" width="100%" border="1" cellpadding="5" cellspacing="0">
							...
							<s:iterator id="untaxInfoList" value="untaxInfoList" status="st1">
								...
								<s:else>
									...
									<td>
										<s:property value="typeno"/></a>
									</td>
									<td>
										<s:iterator id="colorList" value="colorList" status="st3">
											<s:if test="%{colorList[#st3.index].code == untaxInfoList[#st1.index].color}">
												<s:property value="fieldname"/>
											</s:if>
										</s:iterator>
									</td>
									<td>
										<s:if test='%{untaxInfoList[#st1.index].packaging == "0"}'>整箱</s:if>
										<s:elseif test='%{untaxInfoList[#st1.index].packaging == "1"}'>乱尺</s:elseif>
										<s:elseif test='%{untaxInfoList[#st1.index].packaging == "2"}'>样品</s:elseif>
										<s:else>
									...
									<td>
										<s:iterator id="unitList" value="unitList" status="st4">
											<s:if test="%{unitList[#st4.index].code == untaxInfoList[#st1.index].unit}">
												<s:property value="fieldname"/>
											</s:if>
										</s:iterator>
									</td>
