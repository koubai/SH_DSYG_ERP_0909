...
public class PoiWarehouserptYYInDetail extends Poi2007Base {
	...
	public void writeTitle(XSSFSheet sheet, XSSFWorkbook workbook) {
		...
		if (warehouserpt.getTotaltaxamount().compareTo(new BigDecimal(0))< 0 )
			cell.setCellValue("东升盈港入库明细单(退货)");
		else
		...
		if (warehouserpt.getRes04()== null )
			cell_yy.setCellValue("用友供应商编码：" + "");
		else 
			cell_yy.setCellValue("用友供应商编码：" + warehouserpt.getRes04());
