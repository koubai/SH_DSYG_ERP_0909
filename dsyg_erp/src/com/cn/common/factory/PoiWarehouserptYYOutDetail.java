...
public class PoiWarehouserptYYOutDetail extends Poi2007Base {
	...
	public void writeTitle(XSSFSheet sheet, XSSFWorkbook workbook) {
		...
		font_cus.setFontHeightInPoints((short)14);
		XSSFCellStyle style_yy = workbook.createCellStyle();
...
//		XSSFCell cell_no = row4.createCell(8);
		XSSFCell cell_no = row_YY.createCell(8);
		...
		for(int i = 0; i < datas.size(); i++) {
			...
			if(warehouserpt.getListProduct() != null && warehouserpt.getListProduct().size() > 0) {
				...
				for(int j = 0; j < warehouserpt.getListProduct().size(); j++) {
					...
					//if(product.getRes09() != null && !"".equals(product.getRes09())) {
						//cell1.setCellValue(product.getParentid()+product.getRes09());
					//} else {
						cell1.setCellValue(product.getParentid());
					//}
					cell1.setCellStyle(style);
					if (warehouserpt.getRes08()== null)
						cell2.setCellValue("");
					else {
						// 发展
						if (warehouserpt.getRes08().equals("2")){
							cell2.setCellValue(product.getItem16());
						}
						// 贸易
						if (warehouserpt.getRes08().equals("1")){
							cell2.setCellValue(product.getItem18());							
						}
					}
...
*/					
		...
		row = sheet.createRow(num + 9);
		//row.setHeightInPoints((short)18.75);
